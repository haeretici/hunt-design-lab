/**
 * Equipment catalog browser (dual-mode).
 * Source: presets/<mode>/equipment.json
 *
 * uiMode:
...
(function () {
    ...
    const LAYOUT_STORAGE_KEY = 'hdl_equipment_browser_layout';
    ...
    function resolveSpriteUrl(id, genre) {
        ...
        return new URL(
            `assets/sprites/${g}/equipment/alpha/${stem}.png`,
    ...
    function applyUiMode(mode) {
        ...
        if (uiMode === 'view') {
            ...
            if (titleEl && (!init || !init.title)) {
                titleEl.textContent = 'Equipment';
            }
            document.title = 'Equipment wiki — Hunt Design Lab';
        }
    }
    ...
    function parseQuery() {
        ...
        return {
            ...
            slotFilter: params.get('slotFilter') || params.get('slot') || '',
    ...
    function itemWeaponType(item) {
        if (!item || item.weaponType == null) return '';
        return String(item.weaponType).trim().toLowerCase();
    }
    ...
    function itemVocations(item) {
        if (!item || item.vocation == null) return [];
        const v = item.vocation;
        if (Array.isArray(v)) {
    ...
    function matchesSlot(item) {
        ...
        if (target === 'ammunition' || target === 'ammo') {
            return s === 'ammunition' || s === 'ammo' || c === 'ammo' || c === 'quiver';
        }
    ...
    function filteredItems() {
        ...
        const cat = String(categoryFilter || '')
            .trim()
            .toLowerCase();
        const wt = String(weaponTypeFilter || '')
            .trim()
            .toLowerCase();
        ...
        return items.filter((item) => {
            if (!matchesSlot(item)) return false;

            if (cat && itemCategory(item) !== cat) return false;

            if (wt && itemWeaponType(item) !== wt) return false;
    ...
    function sortItems(list) {
        ...
        return list.slice().sort((a, b) => {
            ...
            if (sort === 'category_asc') {
                const ca = itemCategory(a) || '\uffff';
                const cb = itemCategory(b) || '\uffff';
    ...
    function renderTable(list) {
        ...
        body.innerHTML = list
            .map((item) => {
                ...
                const lvl = itemLevel(item);
                const atk = item.atk != null ? Number(item.atk) : null;
                const arm = item.armor != null ? Number(item.armor) : null;
                const def = item.defense != null ? Number(item.defense) : null;
                const slot = item.slot != null ? String(item.slot) : '';
                const cat = item.category != null ? String(item.category) : '';
                const wt = item.weaponType != null ? String(item.weaponType) : '';
                const thumb = imgUrl
                ...
                return `<tr class="${isSel ? 'is-selected' : ''}" data-id="${escapeHtml(item.id)}">
                    ...
                    <td>${slot ? escapeHtml(slot) : '<span class="text-muted">—</span>'}</td>
                    <td>${cat ? escapeHtml(cat) : '<span class="text-muted">—</span>'}</td>
                    <td>${wt ? escapeHtml(wt) : '<span class="text-muted">—</span>'}</td>
                    <td class="eq-num" data-order="${lvl != null ? lvl : -1}">${lvl != null ? lvl : '—'}</td>
                    <td class="eq-num" data-order="${atk != null && !isNaN(atk) ? atk : -1}">${atk != null && !isNaN(atk) ? formatNum(atk) : '—'}</td>
                    <td class="eq-num" data-order="${arm != null && !isNaN(arm) ? arm : -1}">${arm != null && !isNaN(arm) ? formatNum(arm) : '—'}</td>
                    <td class="eq-num" data-order="${def != null && !isNaN(def) ? def : -1}">${def != null && !isNaN(def) ? formatNum(def) : '—'}</td>
                </tr>`;
            })
            .join('');
        ...
        dataTable = new window.DataTable(tableEl, {
            ...
            language: {
                ...
                infoEmpty: '0 items',
                zeroRecords: 'No equipment items match this filter.'
            },
    ...
    function renderGrid(list) {
        ...
        grid.innerHTML = list
            .map((item) => {
                ...
                if (cat) {
                    badges.push(
                        `<span class="eq-badge eq-badge-cat">${escapeHtml(cat)}</span>`
                    );
                }
                if (atk != null && atk > 0) {
                    badges.push(`<span class="eq-badge eq-badge-atk">Atk ${atk}</span>`);
                }
                if (arm != null && arm > 0) {
                    badges.push(`<span class="eq-badge eq-badge-arm">Arm ${arm}</span>`);
                }
                if (def != null && def > 0) {
                    badges.push(`<span class="eq-badge eq-badge-def">Def ${def}</span>`);
                }
    ...
    function renderSidebar() {
        ...
        const speed = selected.speed != null ? Number(selected.speed) : null;
        const wgt = selected.weight != null ? Number(selected.weight) : null;
        const vocs = itemVocations(selected);
        const wt = selected.weaponType != null ? String(selected.weaponType) : '';
        const twoHanded = !!selected.twoHanded;
            ...
            <div class="eq-detail-section">
                ...
                <div class="eq-stat-list">
                    <div class="eq-stat-row">
                        <div class="eq-stat-label"><i class="fa-solid fa-cube text-primary"></i> Slot</div>
                        <div class="eq-stat-val eq-val-slot">${escapeHtml(selected.slot || '—')}</div>
                    </div>
                    <div class="eq-stat-row">
                        <div class="eq-stat-label"><i class="fa-solid fa-tag text-success"></i> Category</div>
                        <div class="eq-stat-val eq-val-cat">${escapeHtml(selected.category || '—')}</div>
                    </div>
                    ${wt ? `
                    <div class="eq-stat-row">
                        <div class="eq-stat-label"><i class="fa-solid fa-hand-fist text-info"></i> Weapon Type</div>
                        <div class="eq-stat-val">${escapeHtml(wt)}</div>
                    </div>` : ''}
                    ${twoHanded ? `
                    <div class="eq-stat-row">
                    ...
                    <div class="eq-stat-row">
                        <div class="eq-stat-label"><i class="fa-solid fa-burst text-warning"></i> Extra Atk</div>
                        <div class="eq-stat-val eq-val-extra-atk">+${extraAtk}</div>
                    </div>` : ''}
                    ${arm != null && arm > 0 ? `
                    <div class="eq-stat-row">
                        <div class="eq-stat-label"><i class="fa-solid fa-shield-halved text-success"></i> Armor Rating</div>
                        <div class="eq-stat-val eq-val-arm">${arm}</div>
                    </div>` : ''}
                    ${def != null && def > 0 ? `
                    <div class="eq-stat-row">
                        <div class="eq-stat-label"><i class="fa-solid fa-shield text-info"></i> Defense</div>
                        <div class="eq-stat-val eq-val-def">${def}</div>
                    </div>` : ''}
                    ${speed != null && speed !== 0 ? `
                    <div class="eq-stat-row">
                        <div class="eq-stat-label"><i class="fa-solid fa-bolt text-warning"></i> Speed Bonus</div>
                        <div class="eq-stat-val eq-val-speed">${speed > 0 ? '+' : ''}${speed}</div>
                    </div>` : ''}
                    ${wgt != null && !isNaN(wgt) ? `
                    <div class="eq-stat-row">
                        <div class="eq-stat-label"><i class="fa-solid fa-weight-hanging text-secondary"></i> Weight</div>
                        <div class="eq-stat-val eq-val-wgt">${(wgt / 100).toFixed(2)} oz</div>
                    </div>` : ''}
                </div>
            </div>
        `;
    ...
    function render() {
        ...
        if (list.length === 0) {
            ...
            if (empty) {
                ...
                empty.hidden = false;
                empty.textContent = 'No equipment items match this filter.';
            }
            updateSelectionChrome();
            renderSidebar();
            return;
    ...
    async function loadItems() {
        ...
        items =
            doc && doc.document && Array.isArray(doc.document.items)
                ? doc.document.items
                : [];
    ...
    function applyInit(payload) {
        ...
        init.slotFilter = init.slotFilter || q.slotFilter;
        ...
        if (init.title) {
            ...
            const titleEl = byId('eqTitle');
            if (titleEl) titleEl.textContent = 'Equipment';
        }

        if (init.slotFilter) {
            slotFilter = String(init.slotFilter);
            const slotEl = byId('eqSlot');
        ...
        loadItems().catch((err) => {
            ...
            if (empty) {
                ...
                empty.textContent =
                    'Failed to load equipment catalog: ' + (err.message || err);
            }
        });
    }
    ...
    function bindEvents() {
        ...
        if (slotEl) {
            slotEl.addEventListener('change', () => {
                slotFilter = slotEl.value;
                render();
            });
        }

        const catEl = byId('eqCategory');
        if (catEl) {
            catEl.addEventListener('change', () => {
                categoryFilter = String(catEl.value || '')
                    .trim()
                    .toLowerCase();
                render();
            });
        ...
        const wtEl = byId('eqWeaponType');
        if (wtEl) {
            wtEl.addEventListener('change', () => {
                weaponTypeFilter = String(wtEl.value || '')
                    .trim()
                    .toLowerCase();
                render();
            });
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (isSelectMode() && window.opener && !window.opener.closed) {
            post({ type: 'ready', kind: 'equipment' });
        }
    });
})();
